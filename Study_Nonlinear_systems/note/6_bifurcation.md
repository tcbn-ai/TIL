...
---
### 2次元ベクトル場のサドル・ノード分岐
ヤコビ行列 (6.22) が1つの単純な実固有値が0になる場合を考える．
2次元ベクトル場
$$
\begin{cases}
  \dot{x} = \mu - x^2 \\
...
$$
を考える．
1. $\mu < 0$のとき: ベクトル場は平衡点を持たない．
2. $\mu = 0$のとき: ベクトル場は$\boldsymbol{x} = (0, 0)$に固有値$0, -1$をもつ平衡点を持つ．線形化行列は
$$
A = \left(
    \begin{array}{cc}
      0 & 0 \\
...
3. $\mu > 0$のとき: ベクトル場は2つの平衡点を持つ．
  - $P^{+} = (+ \sqrt{\mu}, 0)$は固有値$-2 \sqrt{\mu}, -1$を持ち安定 (ノード)
  - $P^{-} = (-\sqrt{\mu}, 0)$は固有値$2\sqrt{\mu}, -1$を持ち不安定

パラメータの変化に伴い，安定平衡点と不安定平衡点とが接近し，合体し，そして消滅する．
$\rightsquigarrow$ サドル・ノード分岐

2次元ベクトル場の場合に，安定結節点 (ノード) と鞍状点 (サドル) との合体になる．
この分岐では，双曲性を崩す0固有値が生じたのは，$x$軸方向の1次元にのみであって，$y$軸方向の安定性は変化しなかった．
$\rightsquigarrow$ 本質的に1次元ベクトル場の分岐に帰着できる．

---

<!--
_footer: '2次元のサドル・ノード分岐: ベクトル場の変化'
-->
![center](../ipython_notebook/6.5_saddle_node_2dim_vector_quiver.svg) 

---

### 2次元ベクトル場のポアンカレ―アンドロノフ―ホップ分岐
ヤコビ行列 (6.22) で，1組の複素共役固有値が純虚数になる場合を考える．
2次元ベクトル場
$$
\begin{cases}
...
\end{cases} \ \ (6.27)
$$
となる．平衡点$\boldsymbol{x} = (0, 0)$における線形化行列は，
$$
A = \left(
    \begin{array}{cc}
      \mu & -\omega \\
...
1. $\mu < 0$のとき: $\boldsymbol{x} = (0, 0)$に複素共役固有値$\mu \pm i \omega$をもつ安定平衡点を持つ．
2. $\mu = 0$のとき: $\boldsymbol{x} = (0, 0)$に共役な純虚数固有値$\pm i \omega$をもつ平衡点を持つ．
3. $\mu > 0$のとき: $\boldsymbol{x} = (0, 0)$に複素共役固有値$\mu \pm i \omega$をもつ不安定平衡点を持つ．その周囲に安定な周期起動を持つ．

---

#### ポアンカレ―アンドロノフ―ホップ分岐
- 複素共役固有値を持つ平衡点において，パラメータの変化に伴って，複素共役固有値が虚軸を横切る．
- 安定平衡点が不安定化し，その周囲に安定周期起動が発生する分岐．
- 時間を反転した力学系を考慮に入れて，不安定平衡点が安定化し，その周囲に不安定周期起動が発生する分岐も同じ名前で呼ばれる．

ポアンカレ―アンドロノフ―ホップ分岐を生じる2次元ベクトル場の標準形は，極座標表示で，
$$
\begin{cases}
  \dot{r} = d \mu r + a r^3 \\
  \dot{\theta} = \omega + c \mu + b r^2
\end{cases} \ \ (6.29)
$$
ただし，$a, b, c, d$および$\omega \neq 0$は定数．
(定数$a, d$の符号の取り方によって，分岐の様子は異なる)

---

<!--
_footer: 'ポアンカレ―アンドロノフ―ホップ分岐: 解軌道の変化 ($a=-1, b = 0, c = 0, d = 1, \omega = 1$) '
-->
![center](../ipython_notebook/6.6_hopf_trajectory.svg) 